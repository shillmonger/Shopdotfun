...
export default function SellerOverviewPage() {
  ...
  const recentOrders = [
    { id: "ORD-1120", buyer: "Alex Johnson", total: 125.00, status: "Pending" },
    { id: "ORD-1118", buyer: "Sarah Smith", total: 85.50, status: "Shipped" },
    { id: "ORD-1115", buyer: "Mike Ross", total: 210.00, status: "Delivered" },
  ];
        ...
        <main className="flex-1 overflow-y-auto pb-32 p-4 md:p-8">
          <div className="max-w-7xl mx-auto space-y-10">
            ...
            <section className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              <div>
                ...
                <div className="flex items-center gap-4 mt-3">
                  <span className={`flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md border ${
                    isVerified 
                    ? "bg-green-500/10 text-green-500 border-green-500/20" 
                    : "bg-amber-500/10 text-amber-500 border-amber-500/20"
                  }`}>
                    <ShieldAlert className="w-3 h-3" /> {isVerified ? "Verified Seller" : "Verification Pending"}
                  </span>
                  {!isVerified && (
            ...
            <section className="space-y-4">
               {!isVerified && (
                  <div className="bg-amber-500 text-black p-5 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4 shadow-lg border-2 border-black/10">
                    <div className="flex gap-4">
                      ...
                      <div>
                        <h3 className="font-black uppercase italic text-sm tracking-tight">Complete your store verification</h3>
               ...
               <div className="bg-foreground text-background p-5 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4">
                  <div className="flex gap-4">
                    <Truck className="w-6 h-6 text-primary shrink-0 mt-1" />
                    <div>
                      <h3 className="font-black uppercase italic text-sm tracking-tight">Ship Pending Orders</h3>
                      <p className="text-[10px] font-bold uppercase opacity-60">You have 3 orders that need to be dispatched within 24 hours.</p>
                    </div>
                  </div>
                  <Link href="/seller/orders" className="w-full md:w-auto bg-primary text-primary-foreground px-6 py-2.5 rounded-lg text-[10px] font-black uppercase tracking-widest">Ship Orders</Link>
               </div>
            </section>
            ...
            <section className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {stats.map((stat, i) => (
                <div key={i} className="bg-card border border-border p-5 rounded-3xl relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8">
                <div className="bg-card border border-border rounded-3xl overflow-hidden">
                  <div className="p-6 border-b border-border flex justify-between items-end">
                    ...
                    <Link href="/seller/orders" className="text-[10px] font-black uppercase text-primary hover:underline">View All Orders</Link>
                  </div>
                  <div className="overflow-x-auto">
                    <table className="w-full text-left">
                      <thead>
                        <tr className="bg-muted/30 border-b border-border">
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-muted-foreground">Order ID</th>
                          <th className="px-6 py-4 text-[10px] font-black uppercase text-muted-foreground">Buyer</th>
                      ...
                      <tbody className="divide-y divide-border">
                        {recentOrders.map((order) => (
                          <tr key={order.id} className="hover:bg-muted/20 transition-colors group">
                            <td className="px-6 py-5 font-black text-sm uppercase tracking-tighter italic">{order.id}</td>
                            <td className="px-6 py-5 text-xs font-bold">{order.buyer}</td>
                            <td className="px-6 py-5 font-black italic">${order.total.toFixed(2)}</td>
                            <td className="px-6 py-5">
                              <span className={`text-[9px] font-black uppercase px-2 py-1 rounded-full border ${
                                order.status === "Pending" ? "border-amber-500/20 text-amber-500 bg-amber-500/5" : "border-green-500/20 text-green-500 bg-green-500/5"
                              }`}>
                                {order.status}
                              </span>
                            </td>
                            <td className="px-6 py-5 text-right">
                              <Link href={`/seller/orders/${order.id}`} className="p-2 inline-block bg-muted rounded-lg group-hover:bg-primary group-hover:text-primary-foreground transition-all">
              ...
              <div className="lg:col-span-4 space-y-8">
                ...
                <section className="bg-card border border-border rounded-3xl p-6">
                  <div className="flex justify-between items-start mb-6">
                    <h2 className="text-xs font-black uppercase tracking-widest">Inventory Alerts</h2>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4 p-3 bg-destructive/5 rounded-2xl border border-destructive/10">
                      ...
                      <div className="flex-1">
                        <p className="text-[10px] font-black uppercase tracking-tighter">Hyper-X Keyboard</p>
                        <p className="text-[9px] font-bold text-destructive uppercase">Only 2 left in stock</p>
                      </div>
                    </div>
                ...
                <section className="bg-foreground text-background rounded-3xl p-6">
                  ...
                  <div className="flex items-end justify-between">
                    ...
                    <span className="text-[10px] font-black uppercase text-primary">Excellent</span>
                  </div>
                  ...
                  <p className="text-[9px] font-medium uppercase mt-4 opacity-50 leading-relaxed">
                    Maintain a high health score to get boosted in search results.
                  </p>
                </section>
              </div>
            </div>
