          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-10">
              <div>
                ...
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-3 flex items-center gap-2">
                  <Layers className="w-3 h-3 text-primary" /> Managing {MY_PRODUCTS.length} Total Listings
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
              {[
                ...
                <div key={i} className="bg-card border border-border p-4 rounded-2xl">
                  ...
                  <p className={`text-2xl font-black italic ${s.color}`}>{s.count}</p>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {MY_PRODUCTS.map((product) => (
                <div key={product.id} className="group bg-card border border-border rounded-[2rem] overflow-hidden hover:border-primary/50 transition-all">
                  ...
                  <div className="relative aspect-square overflow-hidden bg-muted">
                    ...
                    <div className="absolute top-4 left-4">
                      <span className={`text-[9px] font-black uppercase px-3 py-1.5 rounded-full border shadow-sm backdrop-blur-md ${
                  ...
                  <div className="p-6 space-y-4">
                    <div>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <p className="text-lg font-black text-primary italic">
                          {product.price} <span className="text-[10px] not-italic">{product.crypto}</span>
                        </p>
                        <p className="text-[10px] font-bold text-muted-foreground uppercase">SKU: {product.id}</p>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-4 border-t border-border">
                      <div className="flex flex-col">
                        ...
                        <span className={`text-xs font-black ${product.stock < 5 ? 'text-destructive' : ''}`}>
                          {product.stock} Units
                        </span>
                      </div>
                      <div className="flex flex-col">
