...
export default function EditProductPage() {
  ...
  const removeImage = (index: number) => {
    ...
    // If it's an existing image (has publicId), add to removedImageIds
    if (imageToRemove.publicId && !removedImageIds.includes(imageToRemove.publicId)) {
      setRemovedImageIds(prev => [...prev, imageToRemove.publicId!]);
    }
    
    // If it's a new image (has file), remove from imageFiles
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Add removed image public IDs
      removedImageIds.forEach(id => {
        formDataObj.append('removedImages', id);
      });
              ...
              <div>
                <div className="flex items-center gap-4 mb-2">
                  ...
                  <h1 className="text-4xl md:text-6xl font-black uppercase tracking-tighter italic leading-none">
                    Edit <span className="text-primary not-italic">Product</span>
                  </h1>
                </div>
                <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-3 flex items-center gap-2">
                  <Edit2 className="w-3 h-3 text-primary" /> Update your product listing
                </p>
              </div>
              ...
              <div className="lg:col-span-8 space-y-8">
                    ...
                    {formData.images.map((img, idx) => (
                      ...
                      >
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 bg-background/80 rounded-full p-1.5 opacity-0 group-hover:opacity-100 transition-opacity"
                        >
                ...
                <section className="bg-card border border-border rounded-[2rem] p-8">
                  <h3 className="text-xs font-black uppercase tracking-widest flex items-center gap-2 border-b border-border pb-4 mb-6">
                    <Coins className="w-4 h-4 text-primary" /> Pricing & Inventory
                  </h3>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <button className="absolute right-2 top-1/2 -translate-y-1/2 bg-muted hover:bg-muted/80 rounded-lg px-3 py-1.5 text-xs font-bold flex items-center gap-1">
                              {formData.crypto}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                    {PROCESSING_TIMES.map((time) => (
                      <button
                        key={time}
                        type="button"
                        className={`px-4 py-3 rounded-xl text-sm font-bold text-center border transition-colors ${
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-lg font-black">
                          {formData.price ? `${formData.price} ${formData.crypto}` : "0.00"}
                        </span>
                ...
                <div className="bg-card border border-border rounded-2xl p-6 space-y-4">
                  ...
                  <div className="pt-4 border-t border-border space-y-3">
                    <button
                      ...
                      form="product-form"
                      disabled={isSubmitting}
                      className="w-full bg-primary text-primary-foreground py-3 rounded-xl text-sm font-bold flex items-center justify-center gap-2 hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {isSubmitting || isUploading ? (
                        <>
                          <Loader2 className="w-4 h-4 animate-spin" />
                    ...
                    <button
                      type="button"
                      onClick={() => router.back()}
                      className="w-full bg-transparent border border-border py-3 rounded-xl text-sm font-bold hover:bg-muted/50 transition-colors"
                    >
                      Cancel
                    </button>
                  </div>
