...
async function fetchBuyerOrders(): Promise<{ orders: Order[]; stats: { pending: number; shipped: number; total: number } }> {
      ...
      <div className="p-5 md:p-7">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          <div>
            <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">
              {order.orderId} <span className="mx-1.5 opacity-30">|</span> {formatDate(order.createdAt)}
            </p>
          ...
          <div className="flex items-center justify-center gap-2.5 flex-wrap">
            <div className="flex items-center gap-2 bg-muted/50 px-3 py-1.5 rounded-xl border border-border text-[9px]">
              <span className="font-black uppercase opacity-60 tracking-wider">Time</span>
              <span className="font-black uppercase text-primary">{order.productInfo.processingTime}</span>
            </div>
            <span className={`text-[9px] font-black uppercase px-3.5 py-1.5 rounded-xl tracking-wider ${
          ...
          <div className="flex-1 grid sm:grid-cols-2 gap-5">
            ...
            <div className="bg-muted/30 rounded-2xl p-5 border border-border/40 hover:bg-muted/50 transition">
              ...
              <div className="space-y-1.5 text-[10px] font-bold text-muted-foreground uppercase">
                <p className="flex items-center gap-2"><Globe className="w-3.5 h-3.5 opacity-60" /> {order.sellerInfo.country}</p>
                <p className="flex items-center gap-2"><Phone className="w-3.5 h-3.5 opacity-60" /> {order.sellerInfo.phoneNumber}</p>
                <p className="flex items-center gap-2"><Mail className="w-3.5 h-3.5 opacity-60" /> {order.sellerInfo.sellerEmail}</p>
              </div>
            </div>
            ...
            <div className="bg-muted/30 rounded-2xl p-5 border border-border/40">
              ...
              <p className="font-black text-sm uppercase mb-1">{order.buyerInfo.shippingAddress.fullName}</p>
              <p className="text-[10px] font-bold italic uppercase text-muted-foreground leading-snug">
                {order.buyerInfo.shippingAddress.country} , {order.buyerInfo.shippingAddress.city}, {order.buyerInfo.shippingAddress.state}<br />
                {order.buyerInfo.shippingAddress.street},<br />
              </p>
              <p className="mt-1.5 text-[10px] font-black text-primary">{order.buyerInfo.shippingAddress.phone}</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-border flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
          <div className="text-center sm:text-left">
            <p className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.2em] mb-0.5">Amount Paid</p>
            <p className="text-4xl font-black italic tracking-tight">${totalAmount.toFixed(2)}</p>
          </div>
...
export default function BuyerOrdersPage() {
  ...
  const handleAction = (orderId: string, action: string) => {
    toast.info(`${action} Request`, {
      description: `Notification sent for order ${orderId}`,
    });
  };
              ...
              <div className="space-y-6">
                {filteredOrders.map((order) => (
                  <OrderCard
                    ...
                    onProblemChange={(v) => setProblems({ ...problems, [order.orderId]: v })}
